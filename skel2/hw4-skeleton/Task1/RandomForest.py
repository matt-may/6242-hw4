...
class RandomForest(object):
    def __init__(self, num_trees = 10, m = math.sqrt, max_depth = 10,
        ...
        for t in xrange(num_trees):
            self.decision_trees.append(DecisionTree(m=m, max_depth=max_depth,
...
def main():
    ...
    #     for m in [lambda x: 1, lambda x: 5, lambda x: 7, lambda x: 8, lambda x: 9, lambda x: 10, math.sqrt]:
    #         print("i: %d, m: %s" % (i, m(1)))
    #
    #         X_train, X_test, y_train, y_test = cross_validation.train_test_split(X, y)
    #
    #         # Initialize according to your implementation
    #         randomForest = RandomForest(26, m=m)
    #
    #         # Fit the classifier.
    ...
    #
    #         if accuracy > 0.95:
    #             print('added to best forest')
    #             best_forest.append(randomForest)
    #
    #         print "Accuracy: %.4f" % accuracy
    #
    #         fname = "%.4f-%s-fullset" % (accuracy, s)
    #         call(["cp", "cse6242_hw4_submission_May-Matthew.csv", fname])
    #
    # randomForest = RandomForest(9, m=m)
    # randomForest.decision_trees = []
    #
    # for forest in best_forest:
    #     randomForest.decision_trees += forest.decision_trees
    #
    # # Predict results of the test set.
    # y_predicted = randomForest.predict(X_test)
    #
    ...
    # fname = "%.4f-%d-fullset" % (accuracy, i)
    # call(["cp", "cse6242_hw4_submission_May-Matthew.csv", fname])
    from subprocess import call
    import time

    for n in range(25):
        for m in [lambda x: 5, lambda x: 6, lambda x: 7, lambda x: 8, lambda x: 9, lambda x: 10, math.sqrt]:
            print("starting analysis; rf=9; n=%d, m=%s" % (n, m(1)))
            ...
            # fname = "%.4f-%s.pickle" % (accuracy, time.strftime("%I_%M_%S"))
            # print("pickle file: %s" % fname)
            #
            # with open(fname, 'wb') as f:
            #     for tree in randomForest.decision_trees:
            #         pickle.dump(tree.tree, f)

            generateSubmissionFile(myname, randomForest)

            fname = "%.4f-%s-ontestset" % (accuracy, m(1))
            print("submission file: %s" % fname)
            call(["cp", "cse6242_hw4_submission_May-Matthew.csv", fname])

        # print("starting cross val; rf=9; m=%s" % m(1))
        ...
        # import time
        # with open("%.4f-%s.pickle" % (accuracy, time.strftime("%I_%M_%S")), 'wb') as f:
        #     for tree in randomForest.decision_trees:
        #         pickle.dump(tree.tree, f)
        #
        # #generateSubmissionFile(myname, randomForest)
